...
class NHLScreen(SampleBase):
    ...
    def getUpcomingGameScreen(self, game):
        ...
        # draw wins - losses - ot
        graphics.DrawText(offscreen_canvas, font, x+font_width*len(home_team.get_abbreviation())+10, y+50, WHITE, str(home_wins) + "-" + str(home_losses) + "-" + str(home_ot))
        graphics.DrawText(offscreen_canvas, font, x+font_width*len(away_team.get_abbreviation())+10, y+58, WHITE, str(away_wins) + "-" + str(away_losses) + "-" + str(away_ot))
